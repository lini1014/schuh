...
import { SchuhDTO } from '../../src/schuh/controller/schuh-dto.ts';
// @ts-expect-error k6 verwendet esbuild fuer "Type Stripping": import mit "".js" funktioniert nicht
...
// POST /rest
export function postSchuh() {
    const schlagwort =
        schlagwoerter[Math.floor(Math.random() * schlagwoerter.length)];
    const schuh = { ...neuesSchuh };
    schuh.isbn = generateISBN();
    schuh.schlagwoerter = [schlagwort?.toUpperCase() ?? 'N/A'];
    ...
    const response = http.post(restUrl, JSON.stringify(schuh), {
...
// POST /graphql query "schuh"
export function querySchuh() {
